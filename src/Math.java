import java.math.BigDecimal;
import java.math.BigInteger;
import java.math.MathContext;
import java.math.RoundingMode;

public class Math {
    public static final BigDecimal e = new BigDecimal("2.");
    
    public static final RoundingMode DEFAULT_ROUNDING_MODE = RoundingMode.FLOOR;
    public static final int DEC_NUM = 100;
    public static final MathContext mc = new MathContext(DEC_NUM, DEFAULT_ROUNDING_MODE);
    public static final BigDecimal acc = BigDecimal.ONE.divide(bigPow(BigDecimal.valueOf(10), BigInteger.valueOf(DEC_NUM)));

    public static BigDecimal exp(BigDecimal x) {
        BigDecimal res = new BigDecimal(0);
        BigDecimal prevRes = new BigDecimal(1);
        BigInteger n = BigInteger.ZERO;
        while (res.abs().subtract(prevRes.abs()).abs().setScale(DEC_NUM, DEFAULT_ROUNDING_MODE).compareTo(acc) > 0) {
            prevRes = res;

            BigDecimal temp = bigPow(x, n).divide(new BigDecimal(bigFactorial(n)), DEC_NUM + 10, DEFAULT_ROUNDING_MODE); 
            res = res.add(temp);
            n = n.add(BigInteger.ONE);
        }

        return res.setScale(DEC_NUM, DEFAULT_ROUNDING_MODE);
    }

    public static BigDecimal ln(BigDecimal a) {
        BigDecimal x = BigDecimal.ONE.divide(a, mc).subtract(BigDecimal.ONE);
        return mercator(x).round(mc).negate();
    }

    public static BigDecimal mercator(BigDecimal x) {
        BigDecimal res = new BigDecimal(0);
        BigDecimal prevRes = new BigDecimal(1);
        BigInteger n = BigInteger.ONE;
        while (res.abs().subtract(prevRes.abs()).abs().compareTo(acc) > 0) {
            prevRes = res;

            BigDecimal numerator = bigPow(BigDecimal.ONE.negate(mc), n.add(BigInteger.ONE));
            numerator = numerator.multiply(bigPow(x, n));
            res = res.add(numerator.divide(new BigDecimal(n), DEC_NUM + 10, DEFAULT_ROUNDING_MODE));

            n = n.add(BigInteger.ONE);
        }

        return res.setScale(DEC_NUM, DEFAULT_ROUNDING_MODE);
    }

    public static BigDecimal bigPow(BigDecimal b, BigInteger n) {
        b = b.setScale(DEC_NUM + 10, DEFAULT_ROUNDING_MODE);
        BigDecimal res = new BigDecimal("1").setScale(DEC_NUM + 10, DEFAULT_ROUNDING_MODE);
        while (n.compareTo(BigInteger.ZERO) > 0) {
            res = res.multiply(b);
            n = n.subtract(BigInteger.ONE);
        }
        return res;
    }

    public static BigInteger bigFactorial(BigInteger n) {
        BigInteger res = BigInteger.ONE;
        while (n.compareTo(BigInteger.ZERO) > 0) {
            res = res.multiply(n);
            n = n.subtract(BigInteger.ONE);
        }
        return res;
    }

    public static int abs(int x) {
        return (x < 0) ? -x : x;
    }
}
